    ...
    <div className="space-y-5">
      ...
      <div className="relative mt-5 pb-8">
        <svg viewBox="0 0 200 60" className="w-full h-16" preserveAspectRatio="xMidYMid meet">
          <defs>
            <linearGradient id={`secGradient-${selected}`} x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <path
            d={(() => {
              ...
              const mode = min + (max - min) * 0.3; // Peak at 30% of range
              const sigma = (max - min) / 6; // Width of Gaussian
              const lambda = 0.3; // Exponential decay rate
          ...
          {/* Min/Max labels */}
          <text x={(parseInt(data.min) / 50) * 200} y="57" fontSize="8" fill="#6b7280" textAnchor="middle">
            {data.min}
          </text>
          <text x={(parseInt(data.max) / 50) * 200} y="57" fontSize="8" fill="#6b7280" textAnchor="middle">
            {data.max}
          </text>
        </svg>
